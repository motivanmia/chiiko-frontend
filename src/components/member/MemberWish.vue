...
const imageUrl = new URL(
  "@/assets/image/Product/kitchen-knife.jpg",
  import.meta.url
).href;
...
const handleDeleteWish = async (wishId) => {
  try {
    ...
    if (apiResponse.success) {
      ...
      allWish.value = allWish.value.filter(
        (wish) => wish.wishlist_id !== wishId
      );
    } else {
...
<template>
  ...
  <div class="member-collect">
    <div v-for="wish in allWish" :key="wish.wish_id" class="member-card__box">
      <MemberCard
